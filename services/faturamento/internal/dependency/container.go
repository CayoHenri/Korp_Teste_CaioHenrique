...
type Container struct{ HTTPHandler http.Handler }
...
func NewContainer(connection *database.Connection, cfg config.Config) *Container {
	...
	estoqueClient := client.NewEstoqueClient(cfg.EstoqueBaseURL, &http.Client{Timeout: 5 * time.Second})
	...
	return &Container{HTTPHandler: httpPresentation.NewRouter(connection.SQL, handler)}
}
